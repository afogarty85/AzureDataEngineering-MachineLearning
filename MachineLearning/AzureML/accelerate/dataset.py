...
class GroupedDataFrameDataset(Dataset):  
    ...
    def __getitem__(self, idx):  
        ...
        node_id = row['query_NodeId']  
        query_oos_span = row['query_SpellNumber']  
        positive_oos_span = row['positive_SpellNumber']  
        negative_oos_spans = row['negative_SpellNumbers']  
    ...
    def _load_data(self, node_data, oos_span):  
        filters = node_data[node_data['SpellNumber'] == oos_span]  
...
class GroupedInferenceDataset(Dataset):  
    def __init__(self, data_df, cat_cols, binary_cols, cont_cols, label_cols, text_col):  
        ...
        self.mapping = self.data_df.groupby('NodeId')['SpellNumber'].agg(lambda x: set(x)).to_dict()  
    ...
    def __getitem__(self, idx):  
        ...
        if data.empty:  
            print(f"No data found for NodeId: {node_id} and SpellNumber: {oos_span}")  
