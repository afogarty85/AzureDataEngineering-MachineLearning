...
# feature cols
continuous_cols = [c for c in valid_set.columns() if c not in ['SYNDROME', 'test_split', 'CYCLES'] + cat_cols ]
...
options.resource_limits.object_store_memory = None
...
search_space = {
    ...
    "dropout_rates": tune.choice([[0.001, 0.01],
                                  ...
                                  [0.05, 0.05],
                                  [0.01, 0.01],]),
    "batch_size": tune.choice([1024, 1024*2, 1024*3, 1024*4]),
...
tuner = Tuner(trainable=trainer,
              ...
              tune_config=TuneConfig(num_samples=36,  # n trials
