...
def parse_args():
    ...
    parser.add_argument("--use_greedy", default=True, type=lambda x: (str(x).lower() == 'true'), help="If added, use greedy decoding", )
    ...
    def __call__(self, batch):
        ...
        with torch.inference_mode():
            ...
            out = self.model.generate(input_ids=batch['input_ids'],
                            ...
                            **{
                                "max_length": args.ctx_len + 100,
                                "do_sample": False if args.use_greedy else True,
                                "num_beams": None if args.use_greedy else 4,
                                "temperature": None if args.use_greedy else 0.6,
                                "top_p": None if args.use_greedy else 0.9,
                                }
                            )
        ...
        test_ds = ray.data.from_pandas(
            test_ds.to_pandas().assign(
                y_true=test_ds.to_pandas()['input'].map(
                    lambda x: x.split('Output:')[-1].replace('</s>', '').strip(' '))))
    else:
        test_ds = ray.data.from_pandas(
            test_ds.to_pandas().assign(
                y_true=test_ds.to_pandas()['input'].map(
