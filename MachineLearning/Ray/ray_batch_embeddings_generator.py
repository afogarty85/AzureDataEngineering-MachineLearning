...
class EmbeddingGenerator():
    ...
    def __call__(self, batch):
        ...
        x_input_ids = torch.stack([torch.as_tensor(x, device=device)
                                  for x in batch['x_input_ids']], dim=0)
        x_attention_mask = torch.stack([torch.as_tensor(x, device=device)
                                       for x in batch['x_attention_mask']], dim=0)

        y_input_ids = torch.stack([torch.as_tensor(x, device=device)
                                  for x in batch['y_input_ids']], dim=0)
        y_attention_mask = torch.stack([torch.as_tensor(x, device=device)
                                       for x in batch['y_attention_mask']], dim=0)
        ...
        if self.pooling_mode == 'cls':
            ...
            x_sentence_representation = x_sentence_representation.sum(
                dim=1) / x_attention_mask.sum(dim=1)[..., None]

            y_sentence_representation = y_hidden_states.masked_fill(
                ~y_attention_mask[..., None].bool(), 0.0)
            y_sentence_representation = y_sentence_representation.sum(
                dim=1) / y_attention_mask.sum(dim=1)[..., None]
...
def preprocess_function(batch):
    ...
    x_inputs = tokenizer(list(batch['FaultComponent']), max_length=55,
                         truncation=True, padding=True, return_tensors='np')
    y_inputs = tokenizer(list(batch['TechnicianFocus']), max_length=55,
                         truncation=True, padding=True, return_tensors='np')
